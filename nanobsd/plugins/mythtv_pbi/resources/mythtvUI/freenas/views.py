...
from mythtvUI.freenas import forms, models, utils
...
def start(request):
    mythtv_key, mythtv_secret = utils.get_mythtv_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=mythtv_key,
        secret=mythtv_secret)
    ...
    try:
        mythtv = models.MythTV.objects.order_by('-id')[0]
        mythtv.enable = True
        mythtv.save()
    except IndexError:
        mythtv = models.MythTV.objects.create(enable=True)

    try:
        form = forms.MythTVForm(mythtv.__dict__, instance=mythtv, jail=jail)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': 'MythTV data did not validate, please configure it first.',
    ...
    cmd = "%s onestart" % utils.mythtv_control
    _popen = os.popen(cmd)
...
def stop(request):
    ...
    try:
        ...
        mythtv = models.MythTV.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.mythtv_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
...
def edit(request):
    ...
    """
    Get the MythTV object
    If it does not exist create a new entry
    """
    try:
        mythtv = models.MythTV.objects.order_by('-id')[0]
    except IndexError:
        mythtv = models.MythTV.objects.create()
    ...
    if request.method == "GET":
        form = forms.MythTVForm(instance=mythtv,
            jail=jail)
    ...
    form = forms.MythTVForm(request.POST,
        instance=mythtv,
        jail=jail)
    if form.is_valid():
        form.save()
        return JsonResponse(request, error=True, message="MythTV settings successfully saved.")
...
def status(request):
    ...
    proc = Popen(["/usr/bin/pgrep", "-U", "root", "-F", "/var/run/MythTV/MythTV.pid"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request):
    with open(utils.mythtv_icon, 'rb') as f:
        icon = f.read()
